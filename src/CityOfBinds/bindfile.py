...
class BindFile:
    ...
    def publish_file(self, path: str = ''):
        '''Write all the binds to the file.'''
        file_path = Path(path) / self.filename
        with open(file_path, 'w') as file:
            file.write(self._build_banner_string())
            for bind in self._binds:
                file.write(bind.bind_string + '\n')

    def _build_banner_string(self) -> str:
        '''Helper function to build a message string from the binds.'''
        if self.comment_banner:
            return f'##### {self.comment_banner} #####\n'
        return ''
    ...
    def _throw_error_on_invalid_filename(self, filename: str):
        ...
        if ' ' in filename:
            raise ValueError(f'Filename cannot contain spaces: {filename}')
